...
class Lvl(commands.Cog):
            ...
            try:
                ...
                try:
                    with db.cursor() as cursor:
                        cursor.execute("SELECT * FROM `members`.`members` WHERE member_id = %s", member.id)
                        user = cursor.fetchall()[0]
                        lvl  =  user[2]
                        xp =  user[4]
                        cursor.execute("UPDATE `members`.`members` SET exp = %s WHERE member_id = %s", (xp+(0.1), member.id))
                        cursor.execute("SELECT * FROM `members`.`members` WHERE member_id = %s", member.id)
                        xp =  user[4]
                        if xp >= (4*(lvl ** 3)) / 5 :
                            cursor.execute("UPDATE `members`.`members` SET exp = %s, lvl = %s WHERE member_id = %s", (0, lvl+1 ,member.id))
                            lvl = lvl + 1
        ...
        try:
            ...
            try:
                with db.cursor() as cursor:    
                    ...
                    cursor.execute("SELECT * FROM `members`.`members` WHERE member_id = %s", author_id)
                    user = cursor.fetchall()[0]
                    money = user[1]
            ...
            try:
                ...
                try:
                    with db.cursor() as cursor:    
                        ...
                        money = user[1]
                        if money >= 10000:
                            cursor.execute("UPDATE `members`.`members` SET money = %s WHERE member_id = %s", (money - 10000 ,author_id))
                            db.commit()
                            cursor.execute("SELECT * FROM `members`.`members` WHERE member_id = %s", author_id)
                            user = cursor.fetchall()[0]
                            money = user[1]
            ...
            try:
                ...
                try:
                    with db.cursor() as cursor:
                        ...
                        money = user[1] 
                        if money >= 20000:
                            cursor.execute("UPDATE `members`.`members` SET money = %s WHERE member_id = %s", (money - 2000 ,author_id))
                            db.commit()
                            cursor.execute("SELECT * FROM `members`.`members` WHERE member_id = %s", author_id)
                            user = cursor.fetchall()[0]
                            money = user[1]
        ...
        try:
            ...
            try:
                with db.cursor() as cursor:
                    ...
                    user = cursor.fetchall()[0]
                    user_money = user[3]
                    cursor.execute("UPDATE `members`.`members` SET money = %s WHERE member_id =%s", (user_money+money, member.id))
                    db.commit()
    ...
    async def ranke(self,inter: disnake.AppCmdInter, member: disnake.Member = None):
        ...
        money_avatar = Image.open(f'assets/sources/money_avatar.png','r')
        money_lines = Image.open('assets/sources/money_lines.png', 'r').resize((1300+money_len*100, 400))
        money_x = 1427
        money_y = 50
        ...
        font_name= ImageFont.truetype(font = "assets/junegull rg.ttf",size = 56)
        font_text = ImageFont.truetype(font = "assets/junegull rg.ttf", size = 30)
