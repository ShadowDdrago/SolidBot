...
class Mod(commands.Cog): 
    ...
    async def ban (self, inter:AppCmdInter, member: disnake.Member, reason = None ):
        ...
        try:
            ...
            print("Bad")
            print(ex)
        client = Client(host = host, port = port, passwd=passwd) 
        ...
        Mutebackground = Editor(f"assets/Modcard/BANIMBA.png")
        Jungl = Font(path ='assets/junegull rg.ttf', size=40)
    ...
    async def mute(self, inter:AppCmdInter, reason , member: disnake.Member, sec: int = 0, min: int = 0, hours: int = 0 , days: int = 0,  weeks: int= 0):
        ...
        Mutebackground = Editor(f"assets/Modcard/MUTEIMBA.png").resize((1022,1080))
        Jungl = Font(path ='assets/junegull rg.ttf', size=40)
        MutedBro = Editor(f"{member.name}.png").resize((160,160)).circle_image()
    ...
    async def unmute(self,inter:AppCmdInter, member: disnake.Member,reason: str = None):   
        ...
        Mutebackground = Editor(f"assets/Modcard/unmute.png").resize((1022,1080))
        Jungl = Font(path ='assets/junegull rg.ttf', size=40)
        MutedBro = Editor(f"{member.name}.png").resize((160,160)).circle_image()
        ...
        try:
            ...
            try:
                with db.cursor() as cursor:
                    ...
                    warns = user[7]
                    cursor.execute("UPDATE `members`.`members` SET warns = %s WHERE member_id = %s", (warns+1, member.id))
                    db.commit()
                    cursor.execute("SELECT * FROM `members`.`members` WHERE  member_id = %s", member.id)
                    user = cursor.fetchall()[0]
                    warns = user[7]
                    ...
                    if warns == 3:
                        await member.ban()
                        cursor.execute("UPDATE `members`.`members` SET warns = %s WHERE member_id = %s", (2, member.id))
                        db.commit()
                        Mutebackground = Editor(f"assets/Modcard/banwarn.png")
                        Jungl = Font(path ='assets/junegull rg.ttf', size=40)
                        MutedBro = Editor(f"{member.name}.png").resize((160,160)).circle_image()
                        ...
                        Mutebackground = Editor(f"assets/Modcard/warn.png")
                        Jungl = Font(path ='assets/junegull rg.ttf', size=40)
                        MutedBro = Editor(f"{member.name}.png").resize((160,160)).circle_image()
                        Mutebackground.paste(MutedBro.image, (432,427))
        ...
        try:
            ...
            try:
                with db.cursor() as cursor:
                    ...
                    r = requests.get(link, allow_redirects=True)
                    open(filename, "wb").write(r.content)
                    Mutebackground = Editor(f"assets/Modcard/unwarn.png")
                    Jungl = Font(path ='assets/junegull rg.ttf', size=40)
                    MutedBro = Editor(f"{member.name}.png").resize((160,160)).circle_image()
                    Mutebackground.paste(MutedBro.image, (432,427))
